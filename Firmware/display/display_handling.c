...
const unsigned char up_down_icon [] = {
0x10, 0x00, 0x06, 0x00, 
0x20, 0x80, 0x70, 0x80, 0xA8, 0x80, 0x22, 0xA0, 0x21, 0xC0, 0x20, 0x80, 
};
...
void display_handling_draw_main_menu(void)
{
    ...
    else
    {
        ...
        radio_tune_mode_t tune_mode = radio_tune_get_mode();
        if (tune_mode == RADIO_TUNE_MODE_1M)
        {
            draw_image((uint8_t*)up_down_icon, 82, 4);
            lcd_draw_string("1M", 82 + 13, 4, FONT_SIZE_6, 0);
        }
        else if (tune_mode == RADIO_TUNE_MODE_STATIONS)
        {
            draw_image((uint8_t*)up_down_icon, 82, 4);
            lcd_draw_string("STA", 82 + 13, 4, FONT_SIZE_6, 0);
        }
    }
